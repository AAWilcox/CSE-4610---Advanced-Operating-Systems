...
********************************************************************************/
vector<string> Block::block(string buffer, int bSize)
{
	//Number of blocks
	...
	for (int i = 0; i < numberOfBlocks; i++)
	{
		...
		tempblock = buffer.substr(bSize * i, bSize);
	...
	for (int j = blocks[lastBlock].length(); j < bSize; j++)
	{
		blocks[lastBlock] += "#";
	}
