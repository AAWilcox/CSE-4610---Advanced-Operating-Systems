...
/*******************************************************************************
* Purpose:		Saves all of the valid filenames in ROOT to a vector.
* Parameter(s):	None.
* Return:		A vector containing all of the file names in our filesystem.
********************************************************************************/
vector<string> Filesys::ls()
{
	//Vector to hold all the filenames
	vector<string> flist;

	//Iterate through the ROOT's filename vector
	for (int i = 0; i < filename.size(); i++)
	{
		//If there is a valid filename
		if (filename[i] != "XXXXX")
		{
			//Add it to the flist vector
			flist.push_back(filename[i]);
		}
	}

	//Return the flist vector
	return flist;
}